...
async function seed() {
    ...
    const res = await client_1.esClient.bulk({ refresh: true, body });
    if (res.errors) {
        logger_1.logger.error({ res }, "Erro no Bulk");
        process.exit(1);
    }
    logger_1.logger.info("Inclusão dos seeds realizada com sucesso");
}
seed().catch((err) => {
    logger_1.logger.error({ err }, "erros dos dados");
    process.exit(1);
});
