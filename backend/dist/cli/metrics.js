...
async function run() {
    ...
    const searchesAgg = await client_1.esClient.search({
    ...
    const clicksAgg = await client_1.esClient.search({
    ...
    const searchesTotal = searchesAgg.aggregations?.total_searches?.value ?? 0;
    const zero = searchesAgg.aggregations?.zero_results?.doc_count ?? 0;
    const clicksTotal = clicksAgg.aggregations?.total_clicks?.value ?? 0;
    const clicksAt1 = clicksAgg.aggregations?.clicks_rank_1?.doc_count ?? 0;
